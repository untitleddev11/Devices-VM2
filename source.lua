...
LocalPlayer.Character.ChildAdded:Connect(function(Child)
	...
	Child.Activated:Connect(function()
		...
		if Targeting and Targeting.Character then
			...
			if getgenv().CheatSettings.Aiming.MultiParts then
				for Index, PartName in ipairs(getgenv().CheatSettings.Aiming.BodyParts) do
					local Part = Targeting.Character:FindFirstChild(PartName)
				...
				ClosestPart = Targeting.Character:FindFirstChild(getgenv().CheatSettings.Aiming.BodyPart)
			end

			local RootToViewportPoint, IsOnScreen = workspace.CurrentCamera:WorldToViewportPoint(Targeting.Character:FindFirstChild("HumanoidRootPart").Position)
            if getgenv().CheatSettings.Preferences.VisibilityCheck and not IsOnScreen then
                return
            end

            if getgenv().CheatSettings.Preferences.VisibilityCheck and ClosestPart and not Functions:VisibilityCheck(LocalPlayer, Targeting, ClosestPart) then
                return
            end
