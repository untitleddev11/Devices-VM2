...
--> this isnt written by me, credits to depso for this beautiful lib. I had to reupload since it didnt have some i wanted it to have 
...
function ContainerClass:Combo(Config)
    ...
    function Config:SetValue(Value, ...)
        ...
        if type(Value) == "table" then
            Config.Value = Value
            ValueText.Text = table.concat(Value, ", ")
        else
            Config.Value = {Value}
    ...
    function Config:SetOpen(Open: true)
        ...
        if Open then
            Dropdown = ImGui:Dropdown({
                ...
                MultiSelect = true,  -- Enable multi-select dropdown
                SetValue = function(Value)
                    local currentValues = Config.Value
